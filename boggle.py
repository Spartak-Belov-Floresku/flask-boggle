...
class Boggle():
    ...
    def hint(self, board, found_words):
        '''find the first matche word for currant board'''
        for word in self.words:
            if len(word) >= 2 and not word.lower() in found_words:
                if self.check_valid_word(board, word) == "ok":
                    return word.lower()
